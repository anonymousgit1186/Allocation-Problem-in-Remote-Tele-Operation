...
def create_pjt_reg(T, C):
    cur_T = T - C + 1
    cur_C = C - 1
    pjt_stats = pd.read_csv('LP_matching/pjt_stats_3_states.csv')
    pjt = {}
    ...
    for t in range(cur_T, cur_T + cur_C):
        pjt[t] = {}
        for j, scenario in enumerate(scenarios_dict.values()):
            pjt[t][j] = 0.0000001
    return pjt
...
def create_pjt_5_hours(T, C):
    cur_T = T - C + 1
    cur_C = C - 1
    pjt_stats = pd.read_csv('LP_matching/pjt_stats_3_states_5_hours.csv')
    pjt = {}
    for t in range(cur_T):
...
def create_C_lambda(workers):
    return create_C_lambda_reg(workers)


def create_C_lambda_reg(workers):
    C_lambda = dict()
